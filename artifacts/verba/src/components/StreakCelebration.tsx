...
// Celebrazione "streak esteso": overlay full-screen sulla Results (via portal su document.body).
...
.vsc-glow { position: absolute; top: 34%; left: 50%; width: 520px; height: 520px; transform: translate(-50%,-50%) scale(0.6); background: radial-gradient(circle, rgba(245,158,11,0.34), rgba(234,88,12,0.10) 44%, transparent 66%); filter: blur(8px); opacity: 0; pointer-events: none; animation: vsc-glowin 0.7s ease-out forwards; }
.vsc-rays { position: absolute; top: 34%; left: 50%; width: 620px; height: 620px; transform: translate(-50%,-50%); background: repeating-conic-gradient(from 0deg, rgba(251,191,36,0.07) 0deg 3.5deg, transparent 3.5deg 22.5deg); -webkit-mask: radial-gradient(circle, rgba(0,0,0,0.7) 10%, rgba(0,0,0,0.32) 32%, transparent 60%); mask: radial-gradient(circle, rgba(0,0,0,0.7) 10%, rgba(0,0,0,0.32) 32%, transparent 60%); opacity: 0; pointer-events: none; animation: vsc-raysin 0.9s ease-out 0.15s forwards, vsc-spin 40s linear infinite; }
...
.vsc-flamewrap { opacity: 0; margin-bottom: 6px; transform-origin: center bottom; animation: vsc-flamein 0.75s cubic-bezier(0.34,1.56,0.64,1) 0.2s forwards; }
.vsc-flame-inner { display: inline-block; transform-origin: center bottom; animation: vsc-sway 3.4s ease-in-out infinite; }
.vsc-flame-svg { filter: drop-shadow(0 0 20px rgba(245,158,11,0.5)); animation: vsc-heat 2.6s ease-in-out infinite; }
.vsc-numrow { display: flex; align-items: flex-start; gap: 6px; }
.vsc-num { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 96px; line-height: 0.9; color: #FFFFFF; text-shadow: 0 0 26px rgba(245,158,11,0.5); opacity: 0; animation: vsc-numin 0.6s cubic-bezier(0.34,1.56,0.64,1) 0.7s both; }
.vsc-plus { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 27px; color: #F59E0B; margin-top: 12px; opacity: 0; text-shadow: 0 0 14px rgba(245,158,11,0.5); animation: vsc-pop 0.55s cubic-bezier(0.34,1.56,0.64,1) 1.5s both; }
.vsc-label { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 13px; letter-spacing: 0.22em; color: #F59E0B; margin-top: 4px; opacity: 0; animation: vsc-up 0.4s cubic-bezier(0.34,1.56,0.64,1) 1.15s both; }
.vsc-sub { font-family: 'Inter', sans-serif; font-size: 13px; color: rgba(255,255,255,0.55); margin-top: 16px; text-align: center; opacity: 0; animation: vsc-up 0.4s cubic-bezier(0.34,1.56,0.64,1) 1.9s both; }
.vsc-sparks { position: absolute; top: 34%; left: 50%; width: 0; height: 0; z-index: 4; pointer-events: none; }
.vsc-spark { position: absolute; width: var(--sz); height: var(--sz); border-radius: 50%; background: #FBBF24; box-shadow: 0 0 6px rgba(251,191,36,0.8); opacity: 0; animation: vsc-sparkfly 0.95s cubic-bezier(.2,.7,.3,1) var(--delay) forwards; }
.vsc-week { display: flex; gap: 12px; margin-top: 28px; }
...
.vsc-dot.vsc-fill { animation: vsc-dotfill 0.45s cubic-bezier(0.34,1.56,0.64,1) both; }
.vsc-dot.vsc-today { animation: vsc-dotfill-today 0.55s cubic-bezier(0.34,1.56,0.64,1) both; }
.vsc-continue { margin-top: 36px; background: linear-gradient(90deg,#D97706,#F59E0B); color: #fff; font-family: 'Space Grotesk', sans-serif; font-weight: 500; font-size: 15px; padding: 13px 46px; border: none; border-radius: 9999px; cursor: pointer; outline: none; opacity: 0; box-shadow: 0 8px 26px rgba(245,158,11,0.3); animation: vsc-up 0.45s cubic-bezier(0.34,1.56,0.64,1) 2.9s both; }
...
@keyframes vsc-flamein { 0% { opacity: 0; transform: scale(0.2); } 55% { opacity: 1; transform: scale(1.18); } 100% { opacity: 1; transform: scale(1); } }
@keyframes vsc-sway { 0%,100% { transform: rotate(-1.2deg) translateX(-1px); } 50% { transform: rotate(1.2deg) translateX(1px); } }
@keyframes vsc-heat { 0%,100% { filter: drop-shadow(0 0 18px rgba(245,158,11,0.42)); } 50% { filter: drop-shadow(0 0 30px rgba(245,158,11,0.64)); } }
@keyframes vsc-numin { 0% { opacity: 0; transform: scale(1.5); } 60% { opacity: 1; transform: scale(0.96); } 100% { opacity: 1; transform: scale(1); } }
@keyframes vsc-pop { 0% { opacity: 0; transform: scale(0.3) translateY(6px); } 60% { opacity: 1; transform: scale(1.2) translateY(0); } 100% { opacity: 1; transform: scale(1); } }
  ...
  return createPortal(
    <div className="vsc-overlay" role="dialog" aria-label={`Streak: ${streak} giorni`}>
      ...
      <div className="vsc-content">
        ...
        <div className="vsc-numrow">
          <div className="vsc-num">{streak}</div>
          <div className="vsc-plus">+1</div>
        </div>
